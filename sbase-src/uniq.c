...
/* zsh-wasm: see PATCHES.md. prevl holds the previous line's text, so without
 * this a second uniq would compare its first line against the last line of the
 * first one. */
static void
reset_state(void)
{
	countfmt = "";
	dflag = uflag = fskip = sskip = 0;
	prevl.data = NULL;
	prevl.len = 0;
	prevoff = -1;
	prevlinecount = 0;
...
main(int argc, char *argv[])
{
	...
	ARGBEGIN {
	case 'c':
		/* zsh-wasm: BSD's width, not GNU's %7ld. macOS is this project's
		 * reference platform, it is what the zsh shim printed, and it is what
		 * the rendered examples downstream already show. */
		countfmt = "%4ld ";
		break;
	case 'd':
		dflag = 1;
