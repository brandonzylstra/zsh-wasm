...
/* zsh-wasm: see the note in wc.c. Key definitions accumulate on a list across
 * -k options, so a second sort in the same script would otherwise inherit the
 * first one's keys. col1/col2 are scratch buffers and are deliberately kept:
 * they are reused, not leaked. */
static void
reset_state(void)
{
	struct keydef *keydef;

	while ((keydef = TAILQ_FIRST(&kdhead))) {
		TAILQ_REMOVE(&kdhead, keydef, entry);
		free(keydef);
	}
	Cflag = cflag = uflag = 0;
	fieldsep = NULL;
...
main(int argc, char *argv[])
{
	...
	/* zsh-wasm: a global modifier applies to every key, wherever it appears on
	 * the command line. Upstream folds global_flags into a key only at the
	 * moment -k is parsed, so `sort -k2 -n` sorts key 2 lexically while
	 * `sort -n -k2` sorts it numerically. Both GNU and BSD sort treat the two
	 * spellings alike; so does the zsh shim this replaces. -b keeps its
	 * documented restriction to explicit key definitions. */
	TAILQ_FOREACH(keydef, &kdhead, entry)
		keydef->flags |= global_flags & ~(MOD_STARTB | MOD_ENDB);
