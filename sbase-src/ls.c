...
/* zsh-wasm: see PATCHES.md -- a builtin is entered once per call, and ls keeps
 * every one of its flags, its sort mode and its recursion-loop table in
 * file-scope state. */
static void
reset_state(void)
{
	free(tree);
	tree = NULL;
	ret = Aflag = aflag = cflag = dflag = Fflag = fflag = Hflag = 0;
	hflag = iflag = Lflag = lflag = nflag = pflag = qflag = 0;
	Rflag = rflag = Uflag = uflag = 0;
	first = 1;
	sort = 0;
	showdirs = 0;
}
